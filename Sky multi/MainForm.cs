    ...
    internal sealed class MainForm : SkyForms
    {
        ...
        private async void CheckUpdate()
        {
            ...
            if (Environment.Is64BitProcess)
            {
                ...
                if (Sky_Updater.Update.CheckUpdate("Sky multi", CurrentVersion))
                {
        ...
        private async void MouseAndButtonShowAndHide()
        {
            ...
            while (multiMediaViewer != null && this != null)
            {
                ...
                if (Cursor.Position.X != X || Cursor.Position.Y != Y)
                {
                    ...
                    ButtonMediaRight.Location = new Point(this.Width - Border * 2, ButtonMediaRight.Location.Y);
        ...
        private void ImageReader_Click(object sender, EventArgs e)
        {
            ...
            if (DataSettings.Language == Language.French)
            {
                MenuDeroulantMore.NewPage(new string[3] { "Imprimer", "Convertir image", "Définir en tant qu'arrière plan" }, 
                    new MouseEventHandler[3] { new MouseEventHandler(Print), new MouseEventHandler(ConvertImage), new MouseEventHandler(DefineBackground) });
            }
            else
            {
                MenuDeroulantMore.NewPage(new string[3] { "Print", "Convert image", "Set as background" },
                    new MouseEventHandler[3] { new MouseEventHandler(Print), new MouseEventHandler(ConvertImage), new MouseEventHandler(DefineBackground) });
            }
        }
        ...
        private void MultimediaRight()
        {
            ...
            while (GetVideo() == false && GetAudio() == false && multiMediaViewer.ItIsAImage == false || Path.GetExtension(MediaLoaded) == ".nfo");
